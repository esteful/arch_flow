"triangles.grups"<-
  function(x,grup=1,idioma=1,paleta=1,ordenar=1,llegenda=grup, encerclat=1)
  {
    ...
    # encerclat vol dir que el punt té un cercle negre envoltant-lo per defecte, 1, si no es vol es 2-no
    ...
    replayPlot(xxx)
    dev.off()
    #el segon es el triangle ceramic
    par(mar=c(0,0,0,0)+0.1,mgp=c(3,1,0))
    n<-dim(x)[1]
    punts<-matrix(0,n,4)
    plot(c(0,100,50,0),c(0,0,100,0),xlab="",ylab="",axes=F,type="n",xlim=c(-20,120),ylim=c(-20,120))
