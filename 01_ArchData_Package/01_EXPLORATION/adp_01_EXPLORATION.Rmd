...
```{r}
  str(df_raw)
```
In df_chem only integers (int) or nummerical (num). Num class contain decimals 
```{r}
  str(df_chem)
```
...
Most of the sampless are from Gloucester region (n=22), where only one kiln is present. Following byIn Wales, were two kilns are found (Llanedeyrn (n=14) and Caldicot), the latter)  with only two samples. Finally, there are 5 and 5 individuals in the twoboth kilns from New Forest, distributed in the Islands Thorns Kiln and the Ashley Rails kiln.
...
###Boxplots wih the distribution of the chemical elements
```{r}
...
```{r}
  df_raw$Calcium <- c() #Crete the desrired column
...
for (i in 1:nrow(df_chem)){
  ...
  df_chem$SiO2[i] <- 98 - sum(df_chem[i,])


}
}

cbind(df_raw[,1], df_chem) -> df_raw
