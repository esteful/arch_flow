...
if (!require("ggplot2")) {
    install.packages("ggplot2", dependencies = TRUE)
     library(ggplot2)} # Load CRAN Archdata package

if (!require("plyr")) {
    install.packages("plyr", dependencies = TRUE)
     library(plyr)} # Load CRAN Archdata package

if (!require("plyr")) {
    install.packages("plyr", dependencies = TRUE)
     library(dplyr)} # Load CRAN Archdata package
```
...
##Load functions
```{r}
#these are located in R_fun folder, watch the path
...
```{r}
arch_triangles(df_raw, grup =2, SiO= 0) #grup = indicates the column from which the factors will be for the legend 
```
